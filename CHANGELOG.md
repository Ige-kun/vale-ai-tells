...
- **FigurativeCarries** (`ai-tells`): A second documentation audit found the open-subject tokens missing most of the figure in one documentation tree. The determiner list now includes "no," "one," "any," "some," and the possessive ("no set carries the name," "Gravity's badge carries a leading icon"); the subject window takes two modifiers, a hyphen, an apostrophe, or a leading digit inside one, and an adverb before the verb ("the virtual config module carries the option," "an inline suppression always carries an adjacent comment," "a 63-unit brand glyph carries detail"); the cargo tokens' modifier slot takes the same hyphen ("carries a per-illustration prefix"); a code span in or as the subject matches through the asterisk run the parser masks it to ("the `Match` comparison carries the weight," "`Icon` carries its own box"). Four new subject shapes join: the pronoun ("no z-index it carries changes that," "what carries the choice"), an auxiliary before the bare verb ("has to carry the scope," "will not carry the control," "didn't carry before"), the bare plural with no determiner ("Expanded forms carry a negation"), and the plural pronouns on the relative token ("These carry offsets"). "Carries the weight" and "carries some weight" join the weight token. The openings measure zero across the seven pre-LLM corpora except three: the two-modifier window adds three hits, one a plural noun and one a gerund the preposition lookahead now refuses; the pronoun shape adds six human uses of the figure; and the auxiliary shape adds about twenty-five ("does not carry a Go type," "this break can carry a value"), the construction itself, flagged on the maintainer's call, with the two arithmetic uses refused by the lookahead. "Carried on" joins the phrasal exclusions.
...
- **BareNames** (`ai-tells`): The imperative, an instruction to state or give a thing ("Name the version in the changelog," "Name the command behind every reported figure," "name what you tried," "Name every path," "then name the grant this merge goes under"), reports under the designation message, at the start of a sentence or after a colon, a comma, a semicolon, or a coordinator. The plain verb is "state," "give," or "say which." Agent-written skill and runbook prose puts it at three to five per ten thousand words. The seven pre-LLM corpora have it eleven times, every one the literal act of giving a name ("Name the new file `pep-9999.rst`," "Name the logger after the package," "name the result parameter `_`"); a lookahead keeps out the object that a code span, a quoted name, or "after" follows within three words, which reads eight of the eleven, and the three that fire ("Name these methods along the lines of," "name the result parameter, and use a naked return," "Name all the threads to the emitter") are the literal act with the name out of a regex's reach, flagged on the maintainer's call. The message now says to tell the reader to state the thing, and the thirty-nine rule messages that used the verb in their own instruction, whether opening it or following a comma or a coordinator ("Name the actor and use an active verb," "Name the literal action") now say, state, give, use, or describe instead.
...
- **UnderRegime** (`ai-tells`): New rule. The regime opener: a sentence that starts with "Under" and a bare name, making a tool, a decision label, or a layout the jurisdiction the clause holds inside of ("Under cog the same text is configurable twice," "Under mise the mechanism is chosen by where vendir writes the file," "Under layout the file lands in the vendored directory," "Under D15 the endpoint stays on the loopback interface"). The plain form leads with the subject and names the tool after the verb. The gate is the shape: sentence-initial "Under," one bare name with no determiner, then the determiner or pronoun that opens the clause, so "Under this proposal," "Under the hood," and "Under normal circumstances" never match, and the bare condition idioms ("under load," "under test," "under review," "under way," "under fire") sit in the exceptions. The name slot takes a lowercase word, a letter-and-digit label, or a code span, since that is what agent prose puts there; a capitalized proper noun ("Under Windows, the DEFAULT parameter can be used") is the corpora's own platform conditional and stays out. The determiner-led opener is ordinary prose at twenty-nine hits across the seven pre-LLM corpora, nineteen of them Rust RFCs arguing a design; a bare name of any case followed by a determiner matches four times, a platform conditional ("Under Windows, the"), an acronym in a Rust RFC, a Go identifier line the verb exclusions keep out, and an all-caps status note the exceptions hold, and the two openers among them are capitalized, so the case gate takes the count to zero, against four in agent-written documentation of half the size.
- **PointVerdict** (`ai-tells`): New rule. "The point" as a verdict: a copula with "the point" as its complement, grading whatever came before as the purpose of the design ("Speed is the point," "That's the whole point," "This is exactly the point," "Determinism was the point," "The retry is the whole point of the wrapper"), and the intensified purpose statement that fronts the same noun ("The whole point of the cache is to avoid the read"). The reader has just read what the thing does; the sentence adds that this was the intent and says nothing about what the intent bought. The plain form says what the thing does or what it is for. Four rules had read an edge of the figure, `Metacommentary`'s "This is the point," `MicDrop`'s "And that's the point.," `SummativeAppositive`'s comma relative, and `ContrastiveFormulas`' "X isn't the point; Y is," and the demonstrative, the noun subject, and every intensified spelling fell through them. Any subject fires, the modifier slot admits "whole," "entire," "very," "real," and "key," and the lookahead keeps out the literal position ("the point at which," "the point where," "the point following the borrow"), the question ("what is the point?"), and the bare possessive purpose ("the point of the cache"). The copula shape matches twice across the seven pre-LLM corpora, a Go blog post's "this is the key point" and a Rust RFC's "that's the whole point," and "the whole point of" matches eleven times, eight of them Rust RFCs arguing a design ("the whole point of this RFC is to get rid of the embedded flag"), each the construction, so it stays and is flagged.
- **FigurativeMints** (`ai-tells`): New rule. Every form of "mint," banned outright: a step that "mints" an identifier, a workflow that "mints an app token," a "newly minted" release, "the minted installation token," "token minting," keys that "are minted." The determiner gate the verb carried in `MotionMetaphors` read "mints a token" and "minting a short-lived token" and missed the passive, the attributive participle, the gerund noun, the pronoun and bare-plural objects, and the "newly minted" idiom; a survey of agent-written documentation found the figure forty-odd times, almost all the GitHub App token idiom, with the gate reading about half. The verb appears once across the seven pre-LLM corpora, as "Linux Mint 18.3" in a PEP's platform list, which a lookbehind leaves alone, and a lookahead leaves "mint condition," "mint green," and "mint tea" alone too. The literal coinage fires; disable the rule for currency prose. The mint token moved here from `MotionMetaphors`.
...
- **MotionMetaphors**, **EnforcementMetaphors**, **FigurativeQuiet**, **FigurativeRides**, **JourneyMetaphors**, and **FigurativeIdioms** (`ai-tells`): The one-token verbs a documentation-corpus census found no rule reading. `MotionMetaphors` takes the seated passenger and the lagging runner ("the loader seats the record in the cache," "the mirror trails the newest release"); `EnforcementMetaphors` the setting in office and the failure climbing the chain of command ("the flag governs whether the cache writes," "a warning escalates to an error"); `FigurativeQuiet` the muting verb ("the flag silences the warning"); `FigurativeRides` the chain as a ridden mechanism ("the lookup rides the chain"); `JourneyMetaphors` the counted stretch and the tier vocabulary ("walks three tiers," "walks each level of precedence"); and `FigurativeIdioms` burial ("buried in the config," "buries the error"). Governs matches four times across the pre-LLM corpora, escalates five, burial five, silences nine (the Rust RFCs and Python sources writing "silence the lint" and "silence this exception"), and the rest zero.
- **FigurativeNouns** (`ai-tells`): The census's remaining figurative nouns join the list: "grain" as granularity ("one grain of the analysis," "at a coarser grain"), "lens" as a point of view ("through a security lens"), "headline" as the quoted number ("the headline number"), "cascade" as a chain of consequences ("a cascade of retries"), the spatial pair "dead end" and "blind spot," and "seam" as a join between components ("the injection seam"). Each gates on a determiner or a compound and refuses the physical senses ("a grain of sand," "the lens cap," "seam allowance"). Grain matches seven times across the pre-LLM corpora, the Rust RFCs arguing error recovery "at a coarse grain" and a PEP offering "finer grain control," each the figure; the rest match once or twice apiece or not at all. "Strata" stays out as the statistics term for a sampling layer, and "pre-flight" and "heartbeat" stay out as terms of art.
- **FigurativePays** (`ai-tells`): The count as an invoice: a false-positive tally, a match count, or a line count presented as a price ("costs thirteen corpus hits," "the cost is five hits," "at zero corpus hits," "the cost is real"). The maintainer spotted the formula across this package's own rule comments and README rows, the third limb of the "measured cost" family, and the sweep that landed the tokens reworded those to say what matched and how often. Every spelling measures zero across the seven pre-LLM corpora, which report a count as a count.
- **NegatedObject** (`ai-tells`): The perception verbs join the list ("finds no match," "found no examples," "sees no difference"). Both had been measured out on the two standard libraries; a documentation-corpus census showed them everywhere in agent prose, and on the seven pre-LLM corpora the finding verb matches ten times (the Go survey posts writing "found no differences," the runtime "finds no elements to flush") and the seeing verb six times, each the construction itself, so they are flagged on the maintainer's call. The same lookahead drops the degree and idiom phrases ("no more," "no reason," "no way").
- **AnthropomorphicJustification** (`ai-tells`): The standing family gains the intransitive and third-person forms: "stands alone," "stand by itself," "stand on their own," "holds up on its own," "speaks for themselves," and "carries" or "pulls its own weight." "Stand on its own" and "stand on their own" match four times across the seven pre-LLM corpora, each the self-sufficient sense ("the deprecation message should be able to stand on its own"), and the verb phrase "stand alone" matches another four. A lookbehind keeps the adjective ("a stand alone top-level window") out. The rest match nowhere.
...
[1.37.0]: https://github.com/tbhb/vale-ai-tells/compare/v1.36.0...v1.37.0
[1.36.0]: https://github.com/tbhb/vale-ai-tells/compare/v1.35.0...v1.36.0
[1.35.0]: https://github.com/tbhb/vale-ai-tells/compare/v1.34.0...v1.35.0
[1.34.0]: https://github.com/tbhb/vale-ai-tells/compare/v1.33.0...v1.34.0
